...
describe("generateQuotePdf", () => {
  ...
  it("uses default company name when not provided", async () => {
    ...
    expect(text.includes("UK Property Refurb Estimator")).toBe(true);
  });

  it("uses default disclaimer when not provided", async () => {
    const result = await generateQuotePdf(minimalInput);
    const text = Buffer.from(result).toString("latin1");
    expect(text.includes("This estimate is for guidance only")).toBe(true);
  });

  it("includes custom company name when provided", async () => {
    const result = await generateQuotePdf(minimalInput, {
      companyName: "My Test Company"
    });
    const text = Buffer.from(result).toString("latin1");
    expect(text.includes("My Test Company")).toBe(true);
  });

  it("includes custom disclaimer when provided", async () => {
    const result = await generateQuotePdf(minimalInput, {
      disclaimer: "Custom disclaimer for testing."
    });
    const text = Buffer.from(result).toString("latin1");
    expect(text.includes("Custom disclaimer for testing.")).toBe(true);
  });
