...
export default function DashboardGalleryPage() {
  ...
  const [itemPendingDelete, setItemPendingDelete] = useState<GalleryItem | null>(null);
  ...
  const beforePreview = useMemo(
    () => (beforeImageFile ? URL.createObjectURL(beforeImageFile) : form.beforeImageUrl || null),
  ...
  function setField<K extends keyof GalleryFormState>(field: K, value: GalleryFormState[K]) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        ...
        estimatedCost:
          form.estimatedCost.trim().length > 0 ? Number(form.estimatedCost) : undefined
      };

      const endpoint = editingItem ? `/api/v1/gallery/${editingItem.id}` : "/api/v1/gallery";
  ...
  return (
    <section className="space-y-6">
        ...
        <Card className="py-12">
          <CardContent className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-base font-medium text-foreground">
              No project photos yet. Upload your first project to build your portfolio.
            </p>
          </CardContent>
        </Card>
      ) : (
          ...
          {items.map((item) => (
            ...
            >
              <div className="relative h-36 w-full bg-input">
                <NextImage src={item.thumbnailUrl ?? item.imageUrl} alt={item.title} fill className="object-cover" unoptimized />
              </div>
              <div className="space-y-1 p-3">
                <p className="line-clamp-1 text-sm font-medium text-foreground">{item.title}</p>
                <div className="flex items-center justify-between gap-2">
                  <Badge variant="secondary" className="truncate">
                    {item.projectType ? PROJECT_TYPE_LABELS[item.projectType] : "Uncategorised"}
                  </Badge>
                  {item.estimatedCost !== null ? (
                    <span className="text-xs font-mono text-primary">£{item.estimatedCost.toLocaleString("en-GB")}</span>
                  ) : null}
                </div>
              </div>
      ...
      <p className="text-xs text-muted-foreground">Total gallery items: <span className="font-mono">{total}</span></p>
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingItem ? "Edit project photo" : "Add project photo"}</DialogTitle>
            <DialogDescription>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <Button type="button" variant="outline" onClick={() => document.getElementById("gallery-image-input")?.click()}>
                ...
                {imagePreview ? (
                  <div className="relative h-16 w-16 overflow-hidden rounded border border-border">
                    <NextImage src={imagePreview} alt="Project preview" fill className="object-cover" unoptimized />
                  </div>
                ) : null}
              </div>
              {formErrors.image ? <p className="mt-2 text-xs text-destructive">{formErrors.image}</p> : null}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {beforePreview ? (
                  <div className="relative h-16 w-16 overflow-hidden rounded border border-border">
                    <NextImage src={beforePreview} alt="Before preview" fill className="object-cover" unoptimized />
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setField("projectType", value === "unset" ? "" : (value as ProjectType))
                  }
                >
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="bp-focus-ring min-h-24 w-full rounded-md border border-border bg-input px-3 py-2 text-sm text-foreground outline-none focus-visible:ring-0 focus-visible:ring-offset-0"
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This action cannot be undone. The selected gallery item will be removed permanently.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
